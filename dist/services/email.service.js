...
async function zeptoSend(to, subject, html) {
    const body = JSON.stringify({
        from: { address: index_1.config.EMAIL_FROM, name: 'GowdaCommunity' },
    ...
    return new Promise((resolve, reject) => {
        const req = https_1.default.request({
            ...
            headers: {
                ...
                'Authorization': `Zoho-enczapikey ${index_1.config.SMTP_PASS}`,
            },
        }, (res) => {
            let data = '';
...
exports.emailService = {
    async send(job) {
        if (!index_1.config.SMTP_PASS) {
            logger_1.logger.info({ to: job.to, subject: job.subject }, '📧 [DEV EMAIL STUB]');
            return;
        }
        try {
            await zeptoSend(job.to, job.subject, job.html);
            logger_1.logger.info({ to: job.to }, '📧 Email sent');
        }
        catch (err) {
            logger_1.logger.error({ err, to: job.to, subject: job.subject }, '📧 Email send failed');
            throw err;
        }
    },
    // ── Email 1: Welcome Email after Registration ─────────────────────────────
    async sendWelcome(to, displayName, username) {
        const html = `
      <div style="${baseStyle}">
    ...
    async sendProfileApproved(to, displayName) {
        const html = `
      <div style="${baseStyle}">
    ...
    async sendMatrimonyProfileCreated(to, displayName) {
        const html = `
      <div style="${baseStyle}">
    ...
    async sendEventApproved(to, displayName, eventTitle, eventDate, eventLocation) {
    ...
    async sendOtp(to, code, type) {
        const subject = type === 'VERIFY_EMAIL' ? 'Verify your email' :
            type === 'OTP_LOGIN' ? 'Your login code' :
                'Reset your password';
        const action = type === 'VERIFY_EMAIL' ? 'verify your email address' :
      ...
      <div style="${baseStyle}">
        ...
        <div style="padding: 32px 24px;">
          <p style="color: #374151; font-size: 15px; line-height: 1.6; margin: 0 0 20px;">
            Use the code below to ${action}. It expires in <strong>${index_1.config.OTP_EXPIRY_MINUTES} minutes</strong>.
          </p>
    ...
    async sendAdminAlert(to, subject, body) {
        const html = `
      <div style="${baseStyle}">
        ${header('Admin Alert')}
        <div style="padding: 32px 24px;">
