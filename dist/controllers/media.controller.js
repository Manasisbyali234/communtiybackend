...
exports.mediaController = {
    ...
    uploadCommunityImage: (0, asyncHandler_1.asyncHandler)(async (req, res) => {
        if (!req.file)
            throw ApiError_1.ApiError.badRequest('No file provided');
        const result = await media_service_1.mediaService.uploadCommunityImage({ buffer: req.file.buffer, originalname: req.file.originalname, mimetype: req.file.mimetype, size: req.file.size }, req.user.id);
        res.json(new ApiResponse_1.ApiResponse(200, result, 'Community image uploaded successfully'));
    }),
