...
export const archiveBusiness = asyncHandler(async (req: Request, res: Response) => { const current = await prisma.businessListing.findUnique({ where: { id: req.params.id } }); if (!current || (current.userId !== req.user.id && req.user.role !== 'ADMIN')) throw new ApiError(404, 'Business not found'); const row = await prisma.businessListing.update({ where: { id: current.id }, data: { status: 'WITHDRAWN', isVerified: false } }); res.json(new ApiResponse(200, row, 'Business archived')); });
...
export const updateHelp = asyncHandler(async (req: Request, res: Response) => { const current = await prisma.communityHelpRequest.findUnique({ where: { id: req.params.id } }); if (!current || (current.userId !== req.user.id && req.user.role !== 'ADMIN')) throw new ApiError(404, 'Help request not found'); const status = req.user.role === 'ADMIN' ? current.status : (current.urgency === 'URGENT' ? 'APPROVED' : 'PENDING'); const row = await prisma.communityHelpRequest.update({ where: { id: current.id }, data: { category: req.body.category, title: req.body.title, description: req.body.description, location: req.body.location, urgency: req.body.urgency, contactPreference: req.body.contactPreference, status, rejectionReason: null } }); res.json(new ApiResponse(200, row, 'Help request updated')); });
export const archiveHelp = asyncHandler(async (req: Request, res: Response) => { const current = await prisma.communityHelpRequest.findUnique({ where: { id: req.params.id } }); if (!current || (current.userId !== req.user.id && req.user.role !== 'ADMIN')) throw new ApiError(404, 'Help request not found'); const row = await prisma.communityHelpRequest.update({ where: { id: current.id }, data: { status: 'ARCHIVED' } }); res.json(new ApiResponse(200, row, 'Help request archived')); });
